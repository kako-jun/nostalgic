...
async function getDailyCount(db: D1Database, id: string, date: string): Promise<number> {
  ...
  ).bind(`counter:${id}`, date).first<{ count: number }>()
  return row?.count ?? 0
}
...
app.get('/', async (c) => {
  ...
  if (action === 'create') {
    ...
    await db.batch([
      ...
      db.prepare(
        'INSERT INTO counters (service_id, total) VALUES (?, 0)'
  ...
  if (action === 'increment') {
    ...
    ).bind(`counter:${id}`, userHash, today, 'visit').first()
  ...
  if (action === 'display' || action === 'get') {
    const id = c.req.query('id')
    const type = (c.req.query('type') || 'total') as keyof ReturnType<typeof getCounterData> extends Promise<infer T> ? keyof T : never
  ...
  if (action === 'set') {
    ...
    ).bind(`counter:${id}`, hashedToken).first()
    ...
    ).bind(`counter:${id}:total`, numValue, numValue).run()
