...
async function getLikeByUrl(db: D1Database, url: string) {
  ...
  ).bind('like', url).first<{ service_id: string }>()
  if (!mapping) return null
  return db.prepare('SELECT * FROM services WHERE id = ?').bind(`like:${mapping.service_id}`).first()
}
...
async function getTotalLikes(db: D1Database, id: string): Promise<number> {
  ...
  ).bind(`like:${id}:total`).first<{ total: number }>()
  return row?.total ?? 0
}

async function getUserLikeState(db: D1Database, id: string, userHash: string, today: string): Promise<boolean> {
  const row = await db.prepare(
    'SELECT value FROM daily_actions WHERE service_id = ? AND user_hash = ? AND date = ? AND action_type = ?'
  ).bind(`like:${id}`, userHash, today, 'like').first<{ value: string }>()
...
app.get('/', async (c) => {
  ...
  if (action === 'toggle') {
    ...
    ).bind(`like:${id}`, hashedToken).first()
    ...
    if (newState) {
      ...
      await db.batch([
        ...
        db.prepare(
          'UPDATE counters SET total = MAX(0, total - 1) WHERE service_id = ?'
        ).bind(`like:${id}:total`),
      ])
    }
