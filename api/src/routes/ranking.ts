...
async function getRankingByUrl(db: D1Database, url: string) {
  ...
  ).bind('ranking', url).first<{ service_id: string }>()
  if (!mapping) return null
  return db.prepare('SELECT * FROM services WHERE id = ?').bind(`ranking:${mapping.service_id}`).first()
}
...
async function getTopEntries(
  ...
  const { results } = await db.prepare(`
    ...
    LIMIT ?
  `).bind(`ranking:${id}:scores`, limit).all()
...
app.get('/', async (c) => {
  ...
  if (action === 'submit') {
    ...
    ).bind(`ranking:${id}`, hashedToken).first()
    ...
    `).bind(`ranking:${id}:scores`, name, score, displayScore || null, score, displayScore || null).run()

    // Trim excess entries
    const count = await db.prepare(
      'SELECT COUNT(*) as count FROM ranking_scores WHERE service_id = ?'
    ).bind(`ranking:${id}:scores`).first<{ count: number }>()
    ...
    if (count && count.count > maxEntries) {
      ...
      `).bind(`ranking:${id}:scores`, `ranking:${id}:scores`, count.count - maxEntries).run()
    }
  ...
  if (action === 'get') {
    ...
    const limit = Math.min(Number(c.req.query('limit')) || RANKING.LIMIT.DEFAULT, RANKING.LIMIT.MAX)
  ...
  if (action === 'remove') {
    ...
    await db.prepare(
      'DELETE FROM ranking_scores WHERE service_id = ? AND name = ?'
    ).bind(`ranking:${id}:scores`, name).run()
  ...
  if (action === 'clear') {
    ...
    await db.prepare('DELETE FROM ranking_scores WHERE service_id = ?').bind(`ranking:${id}:scores`).run()
