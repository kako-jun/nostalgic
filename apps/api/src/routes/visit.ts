...
async function handleVisitAction(c: AppContext) {
  ...
  // LOOKUP (owner URL lookup - lightweight id check, no counter values payload)
  if (action === "lookup") {
    const url = getParam("url");
    const token = getParam("token");
    ...
    const data = await lookupService(db, "counter", url, token);
    return c.json({ success: true, data });
  }
  ...
  if (action === "batchLookup") {
    ...
    const data = await batchLookupServices(db, "counter", urls, token);
    return c.json({ success: true, data });
  }
  ...
  if (action === "sumByPrefix") {
    ...
    const row = await db
      .prepare(
        "SELECT COALESCE(SUM(total), 0) as total FROM counters WHERE service_id >= ? AND service_id < ? AND service_id LIKE '%:total'"
      )
  ...
  return c.json(
    {
      error:
        "Invalid action. Use: get, lookup, increment, sumByPrefix, create, update, set, delete (GET or POST), batchGet, batchCreate, batchLookup (POST only)",
    },
    400
  );
}
