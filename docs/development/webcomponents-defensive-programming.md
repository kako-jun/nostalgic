...
- DOM操作の安全性
- クラッシュ防止のみ
- API エラー文字列の**表示層ローカライズ**（後述）

### 3. API エラーの表示層ローカライズ（バリデーションではない）

API は常に英語でエラーを返す（単一の真実の源泉）。WebComponents 側は、その英語文字列を
表示する直前に `lang` へ翻訳してよい。これは**表示の責務**であって、バリデーションの肩代わりではない。

- 英語→ローカライズの対訳表 + 動的パターン（残り秒数・文字数を埋め込むもの）を持つ
- 未知のエラー文字列は**原文のままフォールバック**して表示する（握りつぶさない）
- API 側のエラー文言は変えない。クライアントは表示の見た目だけを整える

bbs.js の例: `Please wait 7 seconds before posting again`（連投制限 429）や
`Message must be 420 characters or less` を、フォームのメッセージ欄でローカライズ表示する。
成功時はフォームをクリアし緑系の成功表示を出す。成功/エラーとも約 7 秒間表示する。

### 4. 成功イベントの通知（CustomEvent）

shadow DOM の外（埋め込みページ等）がコンポーネントの成功を知る必要がある場合、
`CustomEvent` を `bubbles: true, composed: true` で host 要素から発火する。
属性追加や DOM 直接操作よりも Web Components として自然で、ページ側は `document` で購読できる。

bbs.js の例: 投稿/編集/削除の成功時に `nostalgic-bbs-posted`（`detail: { id, action }`）を発火し、
単独埋め込みページ（`/bbs?id=...`）がリロード誘導を表示する。
