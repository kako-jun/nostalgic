...
async function showDashboard() {
  ...
  if (services.counter.length > 0) {
    ...
    console.log(
      "│ URL                                    │ ID                   │ Total   │ Today   │ Last Access         │ Status       │"
    );
    console.log(
      "├────────────────────────────────────────┼──────────────────────┼─────────┼─────────┼─────────────────────┼──────────────┤"
    );
    ...
    for (const counter of services.counter.sort((a, b) => a.url.localeCompare(b.url))) {
      ...
      const lastAccess = counter.lastVisit
        ? counter.lastVisit.substring(0, 19).replace("T", " ")
        : "Never";
      const daysAgo = getDaysAgo(counter.lastVisit);
      const status = getStatus(daysAgo);

      console.log(
        `│ ${url.padEnd(38)} │ ${id} │ ${total.padStart(7)} │ ${today.padStart(7)} │ ${lastAccess.padEnd(19)} │ ${status.padEnd(12)} │`
      );
    }
  ...
  if (services.like.length > 0) {
    ...
    console.log(
      "│ URL                                    │ ID                   │ Hearts  │ Last Like           │ Status       │"
    );
    console.log(
      "├────────────────────────────────────────┼──────────────────────┼─────────┼─────────────────────┼──────────────┤"
    );
    ...
    for (const like of services.like.sort((a, b) => a.url.localeCompare(b.url))) {
      ...
      console.log(
        `│ ${url.padEnd(38)} │ ${id} │ ${total.padStart(7)} │ ${lastLike.padEnd(19)} │ ${status.padEnd(12)} │`
      );
    }
  ...
  if (services.ranking.length > 0) {
    ...
    console.log(
      "│ URL                                    │ ID                   │ Entries │ Last Submit         │ Status       │"
    );
    console.log(
      "├────────────────────────────────────────┼──────────────────────┼─────────┼─────────────────────┼──────────────┤"
    );
    ...
    for (const ranking of services.ranking.sort((a, b) => a.url.localeCompare(b.url))) {
      ...
      const lastSubmit = ranking.lastSubmit
        ? ranking.lastSubmit.substring(0, 19).replace("T", " ")
        : "Never";
      const daysAgo = getDaysAgo(ranking.lastSubmit);
      const status = getStatus(daysAgo);

      console.log(
        `│ ${url.padEnd(38)} │ ${id} │ ${entries.padStart(7)} │ ${lastSubmit.padEnd(19)} │ ${status.padEnd(12)} │`
      );
    }
  ...
  if (services.bbs.length > 0) {
    ...
    console.log(
      "│ URL                                    │ ID                   │ Messages │ Last Post           │ Status       │"
    );
    console.log(
      "├────────────────────────────────────────┼──────────────────────┼──────────┼─────────────────────┼──────────────┤"
    );
    ...
    for (const bbs of services.bbs.sort((a, b) => a.url.localeCompare(b.url))) {
      ...
      const lastMessage = bbs.lastMessage
        ? bbs.lastMessage.substring(0, 19).replace("T", " ")
        : "Never";
      const daysAgo = getDaysAgo(bbs.lastMessage);
      const status = getStatus(daysAgo);

      console.log(
        `│ ${url.padEnd(38)} │ ${id} │ ${messages.padStart(8)} │ ${lastMessage.padEnd(19)} │ ${status.padEnd(12)} │`
      );
    }
  ...
  // Summary
  const totalServices =
    services.counter.length + services.like.length + services.ranking.length + services.bbs.length;
  const activeCount = Object.values(services)
    .flat()
    .filter((s) => {
  ...
  console.log(
    `Counters: ${services.counter.length} | Likes: ${services.like.length} | Rankings: ${services.ranking.length} | BBS: ${services.bbs.length}`
  );
