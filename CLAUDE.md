...
```
/{service}?action={action}&url={URL}&token={TOKEN}&...params
```
...
- `/visit` - カウンター（create/increment/display/set/delete）
- `/like` - いいね（create/toggle/get/delete）
- `/ranking` - ランキング（create/submit/get/remove/clear/delete）
- `/bbs` - BBS（create/post/get/update/remove/clear/delete）
...
```bash
# API経由削除（トークンが分かる場合）
curl "https://api.nostalgic.llll-ll.com/visit?action=delete&url={URL}&token={TOKEN}"
curl "https://api.nostalgic.llll-ll.com/like?action=delete&url={URL}&token={TOKEN}"
curl "https://api.nostalgic.llll-ll.com/ranking?action=delete&url={URL}&token={TOKEN}"
curl "https://api.nostalgic.llll-ll.com/bbs?action=delete&url={URL}&token={TOKEN}"
```
...
```
https://api.nostalgic.llll-ll.com/{service}?action=create&url=https://example.com&token=your-secret
```
...
```
# カウントアップ
https://api.nostalgic.llll-ll.com/visit?action=increment&id=your-id

# いいねトグル
https://api.nostalgic.llll-ll.com/like?action=toggle&url=https://example.com&token=your-secret

# スコア送信
https://api.nostalgic.llll-ll.com/ranking?action=submit&url=https://example.com&token=your-secret&name=Player&score=1000

# メッセージ投稿（純粋なGET、1990年代スタイル）
https://api.nostalgic.llll-ll.com/bbs?action=post&url=https://example.com&token=your-secret&author=User&message=Hello
```
